...
function steve(){
	...
	return true;
    return bosses[10]==1 && items.hookshot && (items.hammer || items.glove || items.flippers);
}
...
dungeons[1] = {
    ...
    isBeatable: function(){
		if(!items.glove)
			return "unavailable";
		if(!items.book && !((items.shovelflute==2 || items.shovelflute==3) && items.glove==2 && items.mirror))
			return "unavailable";
...
dungeons[2] = {
    ...
    isBeaten: false,
    isBeatable: function(){
		if((items.shovelflute==0 || items.shovelflute==1) && !items.glove)
			return "unavailable";
...
dungeons[3] = {
    ...
    canGetChest: function(){
		if(!items.moonpearl)
			return "unavailable";
		if(bosses[10]==0 && !(items.hammer&&items.glove) && !(items.glove==2 && items.flippers))
			return "unavailable";
...
dungeons[4] = {
    ...
    canGetChest: function(){
		if(!items.moonpearl || !items.mirror || !items.flippers)
			return "unavailable";
		if(!steve() && !(bosses[10]==1 && items.hammer))
			return "unavailable";
...
dungeons[8] = {
    ...
    isBeaten: false,
    isBeatable: function(){
		if(!items.moonpearl || (items.shovelflute==0 || items.shovelflute==1)|| items.glove!=2 || !items.somaria)
			return "unavailable";
    ...
    canGetChest: function(){
		if(!items.moonpearl || (items.shovelflute==0 || items.shovelflute==1) || items.glove!=2)
			return "unavailable";
		if(!items.boots && !items.hookshot)
			return "unavailable";
		// Medallion Check
...
dungeons[10] = {
    ...
    isBeaten: false,
    isBeatable: function(){
		if(items.lantern && items.sword>0 && (items.sword>1 || items.cape))
			return "available";
		else
			return "unavailable";
    },
...
chests[3] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if ( (items["glove"] || (items.shovelflute==2 || items.shovelflute==3)) && (items["hookshot"] || (items["mirror"]&&items["hammer"])))
		return "available";
	return "unavailable";
    }
};
...
chests[10] = {
    ...
    isOpened: false,
    isAvailable: function(){
		if( (items.shovelflute==2 || items.shovelflute==3) && items["moonpearl"] && items["glove"]==2 )
			return "available";
		return "unavailable";
    }
};
...
chests[16] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( steve() || (bosses[10]==1 && items.moonpearl && items.hammer) )
		return "available";
	return "unavailable";
    }
};
...
chests[30] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( items.sword>=2 && items.book && (items.glove||(items.shovelflute==2 || items.shovelflute==3)) && (items.mirror || (items.hookshot&&items.hammer)) )
		return "available";
	return "unavailable";
    }
};
...
chests[31] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( (steve() || (bosses[10]==1 && items.moonpearl && items.hammer)) && items.mirror && items.sword>=2 && items.book )
		return "available";
	return "unavailable";
    }
};
...
chests[32] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( items.moonpearl && items.glove && (bosses[10]==1 || items.hammer || (items.glove==2 && items.flippers)) )
		return "available";
	return "unavailable";
    }
};
...
chests[34] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( items.glove || (items.shovelflute==2 || items.shovelflute==3) )
		return "available";
	return "unavailable";
    }
};
...
chests[35] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if(items.powdermushroom==2 || items.powdermushroom==3)
		return "available";
	return "unavailable";
    }
};
...
chests[39] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( items.mirror && (steve() || (bosses[10]==1 && items.moonpearl && items.hammer)) )
		return "available";
	return "unavailable";
    }
};
...
chests[41] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( (items.shovelflute==2 || items.shovelflute==3) && items.glove==2 && items.mirror )
		return "available";
	return "unavailable";
    }
};
...
chests[46] = {
    ...
    isOpened: false,
    isAvailable: function(){
		if((items.glove || (items.shovelflute==2 || items.shovelflute==3)) && (items.hookshot || (items.hammer && items.mirror)) )
...
chests[49] = {
    ...
    isAvailable: function(){
		if(items.flippers)
			if( items.moonpearl && items.mirror && (bosses[10]==1 || items.glove==2 || (items.glove&&items.hammer)) )
				return "available";
			else
				return "possible";
		return "unavailable";
...
chests[51] = {
    ...
    isOpened: false,
    isAvailable: function(){
	if( bosses[10]==1 || (items.glove&&items.hammer&&items.moonpearl) || (items.glove==2&&items.moonpearl&&items.flippers) )
			return "available";
		return "unavailable";
    }
};
...
chests[54] = {
    name: "Buried Item <img src='images/shovelflute1.png' class='mini'>",
    x: "14.4%",
    y: "66.2%",
    isOpened: false,
    isAvailable: function(){
...
chests[59] = {
    name: "Mad Batter <img src='images/hammer.png' class='mini'>/<img src='images/mirror.png' class='mini'> + <img src='images/powdermushroom1.png' class='mini'>",
    x: "16.0%",
    y: "58.0%",
    isOpened: false,
    isAvailable: function(){
		if((items.powdermushroom==1 || items.powdermushroom==3) && (items.hammer || (items.glove==2 && items.mirror && items.moonpearl)))
			return "available";
		return "unavailable";
    }
};
...
chests[61] = {
    ...
    isAvailable: function(){
		...
		var crystalCount = 0;
		for(var k=0; k<10; k++)
			if(prizes[k]==4 && bosses[k]==0)
				crystalCount++;
		
		if(!items.moonpearl || crystalCount<2)
			return "unavailable";
		if(items.hammer && (bosses[10]==1 || items.glove))
			return "available";
		if(bosses[10]==1 && items.mirror && steve())
			return "available";
		return "unavailable";
    }
};
...
chests[62] = {
    ...
    isAvailable: function(){
		var pendantCount = 0;
		for(var k=0; k<10; k++)
			if((prizes[k]==1 || prizes[k]==2) && bosses[k]==0)
				if(++pendantCount==3)
					return "available";
        if (items.book)
          return "possible";
